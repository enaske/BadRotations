...
--------------
        ...
        --if talent.acrobaticStrikes then rangeMod = 6 else rangeMod = 0 end
        ...
        local function rtbReroll()
            ...
            if traits.snakeeyes.rank() > 0 then
        ...
        local function bladeFlurrySync()
            return not mode.bladeflurry == 1 or #enemies.yards9 < 2 or buff.bladeFlurry.exists()
        end
...
--------------------
    ...
    -- Adrenaline Rush
            ...
            if isChecked("Adrenaline Rush") and not buff.adrenalineRush.exists() and ttm >= gcd then
                if cast.adrenalineRush("player") then return end
            end    
    -- Ghostly Strike
    ...
    -- Blade Rush
            ...
            if cast.able.bladeRush() and isChecked("Blade Rush") and bladeFlurrySync() and getDistance("target") < 5 and ttm > 1 then
    ...
    -- Action List - Finishers
        local function actionList_Finishers()       
            ...
            if cast.able.betweenTheEyes() and (buff.ruthlessPrecision.exists() or ((traits.deadshot.rank() > 0 or traits.aceupyoursleeve.rank() > 0) and buff.rollTheBones.count > 0)) then
        ...
        -- Roll the Bones
                if not talent.sliceAndDice and cast.able.rollTheBones() then
                    ...
                    if (buff.rollTheBones.remain < 3 or rtbReroll()) and (ttdtarget > 20  or buff.rollTheBones.remain < ttdtarget or ttdtarget > 7777 or buff.rollTheBones.remain == 0) then
                        if cast.rollTheBones("player") then return end
                    end
                end
            end
        -- Between the Eyes
            -- between_the_eyes,if=buff.ruthless_precision.up
            if cast.able.betweenTheEyes() and (traits.deadshot.rank() > 0 or traits.aceupyoursleeve.rank() > 0) then
                 if cast.betweenTheEyes() then return end
            end
        -- Run Through
...
--------------------------------
            ...
            if cast.able.bladeFlurry() and mode.bladeflurry == 1 and #enemies.yards9 >= 2 and not buff.bladeFlurry.exists() then
...
--------------------------
            if inCombat then
                if not IsAutoRepeatSpell(GetSpellInfo(6603)) and getDistance("target") < 5 then StartAttack("target") end
...
--- In Combat - Cooldowns ---
-----------------------------
                if useCDs() and cast.able.sinisterStrike() and attacktar then
...
----------------------------------
    ...
    -- Pistol Shot OOR
                    if cast.able.pistolShot() and isChecked("Pistol Shot out of range") and isValidUnit("target") and #enemies.yards9 == 0 and not stealthingAll and power >= getOptionValue("Pistol Shot out of range") and (comboDeficit >= 1 or ttm <= 1.2) then
                        if cast.pistolShot("target") then return end
                    end
                end
            end -- End In Combat
